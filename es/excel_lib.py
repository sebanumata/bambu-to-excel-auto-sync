"""Shared logic for building/updating impresiones_3d.xlsx from a list of print records."""
import json
...
def rebuild_excel(records: list[dict], xlsx_path: Path) -> None:
    ...
    headers = ["#", "Fecha", "Hora", "Archivo", "Duración", "Impresora", "Placa", "Estado"]
    ws.append(headers)
    ...
    ws2["A4"] = "Éxito"
    ws2["B4"] = '=COUNTIF(Impresiones!H:H,"Éxito")'
    ws2["A5"] = "Cancelado"
    ws2["B5"] = '=COUNTIF(Impresiones!H:H,"Cancelado")'
    ws2["A6"] = "Imprimiendo"
    ws2["B6"] = '=COUNTIF(Impresiones!H:H,"Imprimiendo")'
    ws2["A7"] = "Total"
    ws2["B7"] = "=SUM(B4:B6)"
    ...
    ws2["A9"] = "Primera impresión:"
    ws2["B9"] = "=MIN(Impresiones!B:B)"
    ws2["B9"].number_format = "DD/MM/YYYY"
    ws2["A10"] = "Última impresión:"
    ws2["B10"] = "=MAX(Impresiones!B:B)"
    ws2["B10"].number_format = "DD/MM/YYYY"
    ws2["A11"] = "Total de impresiones:"
    ws2["B11"] = "=COUNTA(Impresiones!A:A)-1"
    ws2["A13"] = "Última sincronización:"
...
def _last_data_row(ws) -> int:
    ...
    scan for the last row that actually has a value in the Archivo column."""
...
def append_missing_records(records: list[dict], xlsx_path: Path) -> int:
    ...
    if "Resumen" in wb.sheetnames:
        wb["Resumen"]["B13"] = datetime.now().strftime("%d/%m/%Y %H:%M")
